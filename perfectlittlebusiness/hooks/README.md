...
Three protective hooks ship with the perfectlittlebusiness plugin. They fire automatically at Claude Code lifecycle events — no setup required after the plugin installs.
...
| `session-end-nudge` | `UserPromptSubmit` | When a substantive session shows farewell signals and `/perfectlittlebusiness:end` hasn't been invoked, injects a single nudge so Claude offers `/perfectlittlebusiness:end` before closing. Fires at most once per session. |
...
After a change, bump the plugin version in `.claude-plugin/plugin.json` and push. Users run `/plugin update perfectlittlebusiness@plb` to pull the new hooks.
