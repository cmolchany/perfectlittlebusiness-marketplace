...
// substantive AND /perfectlittlebusiness:end hasn't been invoked yet, inject additional context that
// suggests Claude offer to run /perfectlittlebusiness:end before closing. Never blocks. Fires at most
// once per session.
...
process.stdin.on('end', () => {
  ...
  if (transcriptPath) {
    ...
    // If /perfectlittlebusiness:end already ran this session, stay silent.
    try {
      const transcript = fs.readFileSync(transcriptPath, 'utf8');
      const lower = transcript.toLowerCase();
      if (lower.includes('/perfectlittlebusiness:end') || lower.includes('"name":"end"') || lower.includes('session closer (self-improvement loop)')) {
        process.exit(0);
      }
  ...
  const message = [
    ...
    'The user looks like they are wrapping up this session and `/perfectlittlebusiness:end` has not been invoked yet.',
    'Before answering, briefly offer to run `/perfectlittlebusiness:end` to:',
